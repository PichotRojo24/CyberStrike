...
export default class LobbyScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.BotonAtras.on("pointerover", () => this.BotonAtras.setScale(1.05));
    this.BotonAtras.on("pointerout", () => this.BotonAtras.setScale(1));

    this.BotonAtras.on("pointerdown", () => {
      this.sound.play("click");
      this.isStartingMatch = false;
      this.leaveQueueAndClose(); // acá sí cerramos
      this.scene.start("MenuScene");
    });

    // Título (elegí el que quieras)
  ...
  getServerHost() {
    // Si abrís el juego desde http://192.168.1.130:8080 -> devuelve 192.168.1.130
  ...
  handleServerMessage(data) {
    switch (data.type) {
      ...
      case "gameStart":
        ...
        // 🔥 clave para que shutdown NO cierre el WS
